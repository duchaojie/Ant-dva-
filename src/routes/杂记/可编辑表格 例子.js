...
export default class EditTable extends Component {
  ...
  textChange(e, record) {
    ...
    const row = rows.find(item => item.key === record.key);
    if (row) {
  ...
  changeToEdit(record) {
    ...
    const cacheData = [...this.state.editCacheData];
    cacheData.push({
      key: record.key,
  ...
  okToDisplay(record) {
    const cacheData = [...this.state.editCacheData];
    const index = cacheData.findIndex((e, i, a) => e.key === record.key);
  ...
  cancelToDisplay(record) {
    ...
    const row = tableData.find(item => item.key === record.key);
    if (row) {
      const editRow = cacheData.find(item => item.key === record.key);
      if (editRow) {
        row.key = editRow.key;
